...
class Environment:
    ...
    def _get_cs_action(self, state: dict) -> str:
        ...
        """
        return self.cs_agent.predict_from_prompt(state, log_probs=True, top_logprobs=len(LABEL))

    def _get_mkt_action(self, state: dict) -> str:
        """
        Get the action to take
        """
        return self.mkt_agent.predict_from_prompt(state, log_probs=True, top_logprobs=len(LABEL))
    ...
    def _record_mkt(
        ...
        self.mkt_records[f"{year}{week}"].append(
            {
                "role": "assistant",
                "content": mkt_action,
            }
    ...
    def _step_mkt(self, year: str, week: str) -> None:
        ...
        self._record_mkt(year, week, mkt_action, mkt_state)
    ...
    def run(self, cs_record_path: str, mkt_record_path: str) -> None:
        """
        Run the environment
        """
